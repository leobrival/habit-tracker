...
// Register Tools
// List Boards
server.registerTool("checker_list_boards", {
...
Returns:
  Array of board objects with id, name, emoji, currentStreak, longestStreak, totalCheckIns`,
    ...
    try {
        const boards = await makeApiRequest("/v1/boards");
...
// Create Board
server.registerTool("checker_create_board", {
...
Returns:
  The created board object with its ID`,
    ...
    try {
        const board = await makeApiRequest("/v1/boards", "POST", {
            name: params.name,
...
// Get Board
server.registerTool("checker_get_board", {
...
Returns:
  Board details including streak, statistics, and configuration`,
    ...
    try {
        const board = await makeApiRequest(`/v1/boards/${params.board_id}`);
...
// Update Board
server.registerTool("checker_update_board", {
...
Returns:
  The updated board object`,
    ...
    try {
        ...
        const board = await makeApiRequest(`/v1/boards/${board_id}`, "PUT", {
            name: updates.name,
...
// Delete Board
server.registerTool("checker_delete_board", {
...
// Quick Check-in
server.registerTool("checker_quick_checkin", {
...
Returns:
  Confirmation with updated streak information`,
    ...
    try {
        const checkin = await makeApiRequest("/v1/quick/check-in", "POST", {
...
// Get Quick Status
server.registerTool("checker_quick_status", {
...
Returns:
  List of all boards with their today's completion status and current streak`,
    ...
    try {
        const status = await makeApiRequest("/v1/quick/status");
...
// List Check-ins
server.registerTool("checker_list_checkins", {
...
Returns:
  List of check-ins with date, amount, and notes`,
    ...
    try {
        const checkins = await makeApiRequest(`/v1/boards/${params.board_id}/check-ins?limit=${params.limit}&offset=${params.offset}`);
...
// Create Check-in
server.registerTool("checker_create_checkin", {
...
Returns:
  The created check-in object`,
    ...
    try {
        const checkin = await makeApiRequest(`/v1/boards/${params.board_id}/check-ins`, "POST", {
            date: params.date,
            amount: params.amount,
            note: params.note,
        });
...
// Get Board Stats
server.registerTool("checker_get_stats", {
...
Returns:
  Statistics including streak, completion rate, and averages`,
    ...
    try {
        const stats = await makeApiRequest(`/v1/boards/${params.board_id}/stats`);
...
// Get Board Heatmap
server.registerTool("checker_get_heatmap", {
...
Returns:
  Array of date/count pairs for heatmap visualization`,
    ...
    try {
        const heatmap = await makeApiRequest(`/v1/boards/${params.board_id}/heatmap?days=${params.days}`);
...
// Get User Profile
server.registerTool("checker_get_profile", {
...
Returns:
  User profile with email, name, timezone, and preferences`,
    ...
    try {
        const user = await makeApiRequest("/v1/users/me");
...
async function main() {
    if (!API_KEY) {
        console.error("WARNING: CHECKER_API_KEY environment variable not set. API calls will fail.");
    }
